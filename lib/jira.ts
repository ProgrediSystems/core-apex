...
class JiraClient {
  ...
  async addTestReportComment(issueKey: string, report: {
    ...
    const statusLabel = report.status === 'passed' ? '[PASS]' : report.status === 'failed' ? '[FAIL]' : '[PARTIAL]';

    // Build test cases list (max 10 shown)
    const testCasesList = report.testCases
      .slice(0, 10)
      .map(tc => `* ${tc.name} ${tc.passed ? '[PASS]' : '[FAIL]'}`)
      .join('\n');
...
----`;
  ...
  async createTestCaseIssues(projectKey: string, parentKey: string, testCases: Array<{
    ...
    for (const tc of testCases) {
      try {
        const statusLabel = tc.status === 'passed' ? '[PASS]' : tc.status === 'failed' ? '[FAIL]' : '[PENDING]';

        const summary = `${statusLabel} ${tc.id}: ${tc.name}`;
...
----
_Generated by Core APEX Test Design Agent_
_Powered by Progredi AI_`;
...
_Powered by Progredi AI_`;
  ...
  async createTestSuiteStory(projectKey: string, suiteName: string, useCase: string, totalTests: number, passRate: number): Promise<JiraIssue> {
    const statusLabel = passRate >= 90 ? '[PASS]' : passRate >= 70 ? '[PARTIAL]' : '[FAIL]';
    const summary = `${statusLabel} Core APEX Tests: ${suiteName} (${passRate}% pass rate)`;
    const description = `*Use Case:* ${useCase}
*Total Tests:* ${totalTests}
*Pass Rate:* ${passRate}%
